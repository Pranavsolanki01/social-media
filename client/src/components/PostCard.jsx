...
const ReplyCard =({reply, user, handleLikes}) => {
  
  return (
    <div className='w-full py-3'>
      <div className='flex gap-3 items-center mb-1'>
        <img 
          src={reply?.userId?.profileUrl ?? NoProfile} alt={reply?.userId?.firstName} 
  ...
  <div className='mb-2 bg-primary p-4 rounded-xl'>
     <div className='flex gap-3 items-center mb-2'>
        ...
        <div className='w-full flex justify-between'>
          <div className=''>
              <Link to={"/profile/" + post?.userId?._id}>
                <p className='text-lg text-ascent-1 font-medium'>
      ...
      <div className='mt-4 flex justify-between items-center px-2 py-2 text-ascent-2 teaxt-base border-[#66666645]'>
                      ...
                      <div className='w-full py-2' key={comment?._id}>
                         <div className='flex gap-3 items-center mb-1'>
                          <Link to={"/profile/" + comment?.userId?._id}>
                            <img 
                              src={comment?.userId?.profileUrl ?? NoProfile} alt={comment?.userId?.firstName}
                              className='w-10 h-10 rounded-full object-cover' 
                              />
                          </Link>
                          <div>
                            ...
                            <span className='text-ascent-2 text-sm'>
                              {moment(comment?.createdAt ?? "2023-10-09").fromNow()}
                            </span>
                          </div> 
                         </div>
                            <div className='ml-12'>
                              ...
                              <div className='py-2 px-8 mt-6'>
                                  {
...

                                    {
                                      showReply === comment?.replies?._id && comment?.replies?.map((reply) => (
                                        ...
                                        handleLikes={() => handleLikes("/posts/likes-comment/" + comment?._id + "/" + reply?._id)}
                                        />
                                      ))
                                    }
                              </div>
